    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (accion.equalsIgnoreCase("cargarTabla")) {
                ...
                es.add("datos", autorDAO.listarTablaAutores());
                out.print(es);
            }
            
            if (accion.equalsIgnoreCase("insertar")) {
                String nombre=request.getParameter("autor");
                out.print(autorDAO.agregarAutor(nombre));
            }
            if (accion.equalsIgnoreCase("eliminar")) {
                int id=Integer.valueOf(request.getParameter("id"));
                out.print(autorDAO.eliminarAutor(id));
            }
            if (accion.equalsIgnoreCase("cargarDatos")) {
                int id=Integer.valueOf(request.getParameter("id"));
                out.print(autorDAO.cargarDatosAutor(id));
            }
            if (accion.equalsIgnoreCase("actualizar")) {
                int id=Integer.valueOf(request.getParameter("id"));
                String nombre=request.getParameter("autor");
                out.print(autorDAO.actualizarAutor(id, nombre));
            }
        }
    }
